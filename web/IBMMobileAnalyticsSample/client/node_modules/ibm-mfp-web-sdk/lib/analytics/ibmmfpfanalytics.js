...
(function(root, factory) {
  ...
  REQ_SEND_LOGS = '/api/loguploader',
    ...
    BUFFER_TIME_IN_MILLISECONDS = 10,
  ...
  var __usingLocalConfiguration = function() {
    var configurationString = analyticsLocalStorage.getItem(
  ...
  function generateUUID(newSession) {
    ...
    if (newSession) {
      uuid = 'xxxxxxxx-xxxx-4567-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,
        generate);
    } else {
  ...
  function initXHR(XHR, analytics) {
    ...
    XHR.prototype.send = function(data) {
      ...
      if (!state.allDomains && (!metadataHeader.contextRoot || this._url.indexOf(
          metadataHeader.contextRoot) == -1)) {
        send.call(this, data);
        return;
      }
      ...
      if (track) {
        ...
        this.setRequestHeader("x-mfp-analytics-metadata", JSON.stringify(
          metadataHeader));
      }
    ...
    return new Promise(function(resolve, reject) {
      ...
      __ajax(data, REQ_SEND_LOGS)
        .then(function(response) {
          ...
          console.log(
            'analytics: Client logs successfully sent to the server'
          );
          resolve('Log was successfully sent');
        })
        .catch(function(err) {
          console.error('analytics: Call failed, server returned: ',
            err.statusText);
  ...
  var __persistLog = function(log, key) {
    ...
    try {
      ...
      console.log(
        'analytics: Local storage capacity reached. Client logs will not be persisted'
      );
    }
  };
  ...
  var __attemptFileSwap = function() {
    try {
      var currentLogs = analyticsLocalStorage.getItem(
        KEY_LOCAL_STORAGE_LOGS);
      analyticsLocalStorage.setItem(KEY_LOCAL_STORAGE_SWAP, currentLogs);
      analyticsLocalStorage.removeItem(KEY_LOCAL_STORAGE_LOGS);
    } catch (e) {
      console.log(
        'analytics: Local storage capacity reached. WL.Logger will delete old logs to make room for new ones.'
      );
      analyticsLocalStorage.removeItem(KEY_LOCAL_STORAGE_LOGS);
  ...
  var __processUpdateConfig = function(configString) {
    ...
    if (config && config.clientLogProfileConfig) {
      console.log(
        'analytics: Matching configuration successfully retrieved from the server.'
      );
      var wllogger = config.clientLogProfileConfig;
      analyticsLocalStorage.setItem(KEY_REMOTE_STORAGE_CONFIG,
        analyticsLocalStorage.getItem(KEY_LOCAL_STORAGE_CONFIG));
      state.levelFromServer = null;
      state.filtersFromServer = null;
      __setServerOverrides(wllogger.clientLogProfiles);
    } else {
      console.log(
        'analytics: No matching configurations found from the server. Defaulting to local configuration'
      );
  ...
  printStackTrace.implementation.prototype = {
    ...
    mode: function(e) {
      if (e["arguments"] && e.stack) {
        ...
        return "ie"
      } else if (typeof e.message === "string" && typeof window !==
    ...
    deinstrumentFunction: function(e, t) {
      if (e[t].constructor === Function && e[t]._instrumented && e[t]._instrumented
    ...
    chrome: function(e) {
      var t = (e.stack + "\n").replace(/^\S[^\(]+?[\n$]/gm, "").replace(
        ...
        /^Object.<anonymous>\s*\(([^\)]+)\)/gm, "{anonymous}()@$1").split(
        "\n");
      t.pop();
      return t
    },
    safari: function(e) {
      return e.stack.replace(/\[native code\]\n/m, "").replace(
          /^(?=\w+Error\:).*$\n/m, "").replace(/^@/gm, "{anonymous}()@")
        .split("\n")
    },
    ie: function(e) {
      var t = /^.*at (\w+) \(([^\)]+)\)$/gm;
      return e.stack.replace(/at Anonymous function /gm,
    ...
    opera11: function(e) {
      ...
      for (var s = 0, o = r.length; s < o; s += 2) {
        ...
        if (u) {
          ...
          f = f.replace(/<anonymous function: (\S+)>/, "$1").replace(
    ...
    opera10a: function(e) {
      ...
      for (var s = 0, o = r.length; s < o; s += 2) {
        ...
        if (u) {
          var a = u[3] || t;
          i.push(a + "()@" + u[2] + ":" + u[1] + " -- " + r[s + 1].replace(
            /^\s+/, ""))
        }
      }
      return i
    ...
    opera9: function(e) {
      ...
      for (var s = 2, o = r.length; s < o; s += 2) {
        var u = n.exec(r[s]);
        if (u) {
          i.push(t + "()@" + u[2] + ":" + u[1] + " -- " + r[s + 1].replace(
            /^\s+/, ""))
        }
      }
      return i
        ...
        if (i === undefined) {
          ...
          if (i.constructor === Array) {
            if (i.length < 3) {
              ...
              t[r] = "[" + this.stringifyArguments(n.call(i, 0, 1)) +
                "..." + this.stringifyArguments(n.call(i, -1)) + "]"
            }
    ...
    isSameDomain: function(e) {
      return typeof location !== "undefined" && e.indexOf(location.hostname) !==
        -1
    },
  ...
  function formatStackLine(stackLine) {
    var formats = [/^\x20*at\x20([^(]+)\x20\(?(.*?)(?::(\d+):(\d+))?\)$/,
      /^([^@]*)@(.*?)(?::(\d+):(\d+))?\)?$/,
      /^\x20+at\x20(.*?)(?::(\d+))?$/, /^(.*?)(?::(\d+):(\d+))?$/
    ];
    var method = "";
  ...
  function __setState(options) {
    ...
    state = {
      enabled: typeof options.enabled === 'boolean' ? options.enabled : state
        .enabled,
      stringify: typeof options.stringify === 'boolean' ? options.stringify : state
        .stringify,
      pretty: typeof options.pretty === 'boolean' ? options.pretty : state
        .pretty,
      stacktrace: typeof options.stacktrace === 'boolean' ? options.stacktrace : state
        .stacktrace,
      ismsie: typeof options.ismsie === 'boolean' ? options.ismsie : state
        .ismsie,
      ...
      filters: options.filters === null || typeof options.filters ===
        'object' ? options.filters : state.filters, // {'jsonstore': 'WARN', 'otherPkg': 'DEBUG'}
      filtersFromServer: typeof options.filtersFromServer === 'object' ?
        options.filtersFromServer : state.filtersFromServer,
      ...
      capture: typeof options.capture === 'boolean' ? options.capture : state
        .capture,
      captureFromServer: typeof options.captureFromServer === 'boolean' ?
        options.captureFromServer : state.captureFromServer,
      analyticsCapture: typeof options.analyticsCapture === 'boolean' ?
        options.analyticsCapture : state.analyticsCapture,
      allDomains: typeof options.allDomains === 'boolean' ? options.allDomains : state
        .allDomains,
      maxFileSize: typeof options.maxFileSize === 'number' && options.maxFileSize %
        1 === 0 ? options.maxFileSize : state.maxFileSize,
      autoSendLogs: typeof options.autoSendLogs === 'boolean' ? options.autoSendLogs : state
        .autoSendLogs
    };
  ...
  function __stringify(input) {
    ...
    if (input instanceof Error) {
      return (state.stacktrace) ? printStackTrace({
        e: input
      }).join('\n') : input.toString();
    } else if (typeof input === 'object' && JSON && JSON.stringify) {
      try {
        return (state.pretty) ? JSON.stringify(input, null, ' ') : JSON.stringify(
          input);
      } catch (e) {
  ...
  function __log(args, priority) {
    ...
    if (typeof console === 'object') { // avoid infinite loop on Adobe AIR
      ...
      if (typeof console[priority] === 'function') {
        (state.stringify) ? console[priority](str): console[priority].apply(
          console, args);

      } else if (priority === 'fatal') {
        if (typeof console.error === 'function') {
          (state.stringify) ? console.error(str): console.error.apply(
            console, args);
        }

      } else if (priority === 'trace') {
        if (typeof console.debug === 'function') {
          (state.stringify) ? console.debug(str): console.debug.apply(
            console, args);
        }
        ...
        (state.stringify) ? console.log(str): console.log.apply(console,
          args);
  ...
  function __sendAll(checkAutoSend) {
    ...
    return __send([KEY_LOCAL_STORAGE_ANALYTICS, KEY_LOCAL_STORAGE_LOGS,
      KEY_LOCAL_STORAGE_SWAP
    ]);
  };
  ...
  function _updateConfigFromServer() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      var getConfigUrl = REQ_UPDATE_CONFIG + '/' + appName + '/' +
        platform;
  ...
  function __setServerOverrides(configFilters, level, capture) {
    ...
    _config({
      levelFromServer: level,
      captureFromServer: capture,
      filtersFromServer: updatedConfigFilters
    });
  };
  ...
  function _init(deviceID, appName, contextRoot) {
    ...
    try {
      if (typeof(Storage) !== 'undefined') {
        ...
        if (__usingLocalConfiguration()) {
          configurationString = analyticsLocalStorage.getItem(
            KEY_LOCAL_STORAGE_CONFIG);
        } else {
          configurationString = analyticsLocalStorage.getItem(
            KEY_REMOTE_STORAGE_CONFIG);
        ...
        if (configurationString === null) {
          ...
          var stateString = JSON.stringify(state);
          analyticsLocalStorage.setItem(KEY_LOCAL_STORAGE_CONFIG,
            stateString);
        } else {
          var configuration = JSON.parse(configurationString);
          __updateState(configuration);
  ...
  function capture(captureLogs) {
    _config({
      capture: typeof(captureLogs) === "boolean" ? captureLogs : true
    });
  };

  function __enable(enableLogs) {
    _config({
      enabled: typeof(enableLogs) === "boolean" ? enableLogs : true
    });
  };
  ...
  var _enable = function(analyticsCapture) {
    _config({
      analyticsCapture: typeof(analyticsCapture) === "boolean" ?
  ...
  function _enableAutoSend(autoSend) {
    _config({
      autoSendLogs: typeof(autoSend) === "boolean" ? autoSend : true
    });
  };
